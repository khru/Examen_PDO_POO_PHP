<?php
	...
	class Validaciones
	{
		...
		public static function saneamiento($parm){
			/*if (is_array($parm)) {
				foreach ($parm as $clave => $valor) {
					$valor= self::saneamiento($valor);
				}
			}*/
			...
			if (isset($img["img_perf"])) {
		 		...
		 		if ($img["img_perf"]["size"] > $tam_max) {
		 			...
		 			if(!$mime){
		 				...
		 				$mimereal = finfo_file($mimeinfo, $img['archivo']['tmp_name']);
		 				if (!self::tipoFichero($mimereal)) {
		...
		public static function subirImg($id_con, $img){
			...
			$trozos = explode('/', $img['img_perf']['type']);
			$extension = $trozos[1];
			$destino = $dir  . $id_con . '.' . $extension;
			// creamos el directorio si da error mostramos un error
			if(self::crearDir($dir)){
				if (!move_uploaded_file($img["img_perf"]["tmp_name"],$destino)) {
					$errores["img_perf"] = "Error guardando la imagen";
				} else {
					// añadimos la ruta a $_SESSION con el destino
		...
		public static function validarPassAlta($pass1, $pass2)
		{
			...
			if (!isset($pass2) || empty($pass2)) {
				$errores["pass2"] = "La contraseña está vacía";
			} elseif (mb_strlen(trim($pass2)) == 0) {
				$errores["pass2"] = "La contraseña está vacío";
			} elseif(mb_strlen(trim($pass2)) < 6) {
				$errores ["pass2"] = "La contraseña es demasiado corta";
			} elseif (mb_strlen(trim($pass2)) > 25) {
				$errores ["pass2"] = "La contraseña es demasiado larga";
			} elseif(!self::regexMayusculas($pass2) || !self::regexMinusculas($pass2) || !self::regexNumeros($pass2)) {
				$errores["pass2"] = "La contraseña no cumple con el formato";
			}
		...
		// =====================================================================================
	 	...
	 	public static function regexDireccion($cadena, $longitud = 100){
	 		$pattern = "/^[A-Za-zñÑáéíóúÁÉÍÓÚàèìòùÀÈÌÔÙçÇ 0-9 -#\\/]{3," . $longitud . "}$/";
	 		if (preg_match($pattern, $cadena)) {
	 			return true;
	 		}
	 		return false;
